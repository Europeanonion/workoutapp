(() => {
    // Add debounced functions at the top of IIFE
    const debouncedUpdateHistory = debounce(updateHistoryTable, 250);
    const debouncedSaveProgress = debounce(saveProgress, 500);
    const debouncedDisplayWorkout = debounce((data) => window.displayWorkout(data), 100);
    ...
    function bindUIEvents() {
        ...
        if (filterInput) {
            filterInput.addEventListener('input', debouncedUpdateHistory);
        }
    ...
    async function handlePhaseChange() {
        ...
        // Load new phase
        await loadWorkout(selectedPhase);
    }
    ...
    function saveProgress(index, exerciseName) {
        ...
        const sessionData = {
            ...
            date: new Date().toISOString(),
            phase: document.getElementById("phase-selector").value
        };
